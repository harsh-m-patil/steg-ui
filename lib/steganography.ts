 ...
 */
export async function encodeMessage(image: File, message: string, technique = "lsb"): Promise<string> {
 ...
 */
export async function decodeMessage(image: File, technique = "lsb"): Promise<string> {
  return new Promise((resolve, reject) => {
    const reader = new FileReader()
    reader.onload = () => {
      const img = new Image()
 ...
 */
function processEncoding(img: HTMLImageElement, message: string, technique: string): string {
  ...
  if (binaryMessage.length > maxBits) {
    throw new Error(`Message too large for this image. Maximum size: ${Math.floor(maxBits / 8)} bytes.`)
  }
...
function processDecoding(img: HTMLImageElement, technique: string): string {
  ...
  switch (technique) {
    ...
    case "patchwork":
      ...
      break
    case "dct":
      binaryMessage = extractDCTSteganography(data, canvas.width, canvas.height)
      break
    case "histogram":
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    for (let j = 0; j < 3; j++) {
      if (bitIndex < binaryMessage.length) {
        ...
        data[i + j] = (data[i + j] & 0xfe) | Number.parseInt(binaryMessage[bitIndex])
        bitIndex++
      } else {
 ...
 * @param binaryMessage - The binary message to hide
 */
function applyImprovedLSB(data: Uint8ClampedArray, binaryMessage: string): void {
  let bitIndex = 0
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    if (bitIndex < binaryMessage.length) {
      ...
      data[i] = (data[i] & 0xfd) | (Number.parseInt(binaryMessage[bitIndex]) << 1)
      bitIndex++
    }

    if (bitIndex < binaryMessage.length) {
      // Green channel - use LSB
      data[i + 1] = (data[i + 1] & 0xfe) | Number.parseInt(binaryMessage[bitIndex])
      bitIndex++
    }

    if (bitIndex < binaryMessage.length) {
      // Blue channel - use 3rd LSB
      data[i + 2] = (data[i + 2] & 0xfb) | (Number.parseInt(binaryMessage[bitIndex]) << 2)
      bitIndex++
    }
 ...
 */
function applyPatchwork(data: Uint8ClampedArray, binaryMessage: string, width: number, height: number): void {
  let bitIndex = 0
 ...
 * @returns The binary message extracted from the image
 */
function extractPatchwork(data: Uint8ClampedArray, width: number, height: number): string {
  let binary = ""
  const patchSize = 8 // Size of each patch
  ...
  for (let bitIndex = 0; bitIndex < maxBits; bitIndex++) {
    ...
    for (let y = -1; y <= patchSize; y++) {
      for (let x = -1; x <= patchSize; x++) {
        ...
        if (patchY + y >= 0 && patchY + y < height && patchX + x >= 0 && patchX + x < width) {
          const pixelIndex = ((patchY + y) * width + (patchX + x)) * 4
    ...
    const surroundingBlueAvg = surroundingPixelCount > 0 ? surroundingBlueSum / surroundingPixelCount : blueAvg
...
function applyDCTSteganography(data: Uint8ClampedArray, binaryMessage: string, width: number, height: number): void {
  let bitIndex = 0
  const blockSize = 8
  ...
  // Loop through blocks
  for (let blockY = 0; blockY < blocksY && bitIndex < binaryMessage.length; blockY++) {
    for (let blockX = 0; blockX < blocksX && bitIndex < binaryMessage.length; blockX++) {
      ...
      for (let i = 0; i < embedPositions.length && bitIndex < binaryMessage.length; i++) {
 ...
 * @returns The binary message extracted from the image
 */
function extractDCTSteganography(data: Uint8ClampedArray, width: number, height: number): string {
  let binary = ""
  const blockSize = 8
  ...
  for (let i = 0; i < 16; i++) {
    ...
    messageLengthBinary += Math.round(metadctCoeffs[row][col]) % 2 === 0 ? "0" : "1"
  }
  ...
  // Loop through blocks
  for (let blockY = 0; blockY < blocksY && binary.length < messageLength; blockY++) {
    for (let blockX = 0; blockX < blocksX && binary.length < messageLength; blockX++) {
      ...
      for (let i = 0; i < embedPositions.length && binary.length < messageLength; i++) {
        const [u, v] = embedPositions[i]
...
function applyHistogramShifting(data: Uint8ClampedArray, binaryMessage: string): void {
  ...
  // Skip the first 32 pixels (metadata area)
  for (let i = 32 * 4; i < data.length && bitIndex < binaryMessage.length; i += 4) {
...
function extractHistogramShifting(data: Uint8ClampedArray): string {
  ...
  for (let i = 32 * 4; i < data.length && binary.length < messageLength; i += 4) {
    const pixelValue = data[i + channel]

    if (pixelValue === peakPoint) {
      // If pixel value is at peak point, bit is 0
      binary += "0"
    } else if ((shiftRight && pixelValue === peakPoint + 1) || (!shiftRight && pixelValue === peakPoint - 1)) {
